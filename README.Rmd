...
```{r figure_A1, fig.dim=c(5, 5), out.width="50%"}
as_tibble(plot_obj[[7]][c("x", "se", "fit")]) %>%
  ggplot(aes(x = x)) +
    ...
    geom_ribbon(aes(ymin = fit - se, ymax = fit + se, y = NULL),
                fill = "grey80") +
    geom_line(aes(y = fit)) +
    labs(x = "Survey year", y = "centered MHI-5 score") +
    theme_minimal() +
    theme(panel.grid = element_blank())
```
...
```{r figure_A2, warning=F, fig.dim=c(4, 8), out.height="50%", out.width="50%"}
p1 + p2
```
...
```{r figure_A3, warning=F, fig.dim=c(6,6), out.width="66%"}
hilda.data %>%
  filter(!topup) %>%
  filter(cohort %in% c("1940s", "1950s", "1960s", "1970s", "1980s", "1990s")) %>%
...
```{r figure_A5, fig.dim=c(6, 6), out.width="75%"}
...
```{r figure_A6, fig.dim=c(6,6), out.width="66%"}
hilda.data %>%
  filter(!topup) %>%
  filter(cohort %in% c("1940s", "1950s", "1960s", "1970s", "1980s", "1990s")) %>%
  ggplot(aes(x = age, y = ghmh)) +
...
```{r figure_A7, fig.dim=c(6,6), out.width="66%"}
hilda.data %>%
  filter(cohort %in% c("1940s", "1950s", "1960s", "1970s", "1980s", "1990s")) %>%
  group_by(xwaveid) %>%
