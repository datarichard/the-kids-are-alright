...
This README describes the data preprocessing, main analysis and figure
construction used in the paper titled “The kids are not ok: Differential
trends in mental health in Australia”.

The following R libraries are required to perform the main analysis and
create figures 1 & 2.
...
\|–– preprocessed.RDS *(preprocessed data after
...
After application was approved, we downloaded the combined dataset
comprising the following 20 files:
...
(GAMM) setting, described by Wood et al (1–4). This is an analogue to a
linear multilevel model with varying intercepts and slopes among the
cohorts, but here the slopes are allowed to “wiggle”. The model includes
a global smoothing term for the effect of age as well as cohort-specific
terms, so each cohort is allowed to have it’s own functional response
but the penalty ensures that functions too far from average are
penalized.
...
The smooth trends were centered for identifiability reasons (5, 6),
however the resulting model estimation allowed two important
comparisons: Firstly the mean MHI-5 estimates
(![\\beta\_{\[k\]}](https://latex.codecogs.com/png.image?%5Cdpi%7B110%7D&space;%5Cbg_white&space;%5Cbeta_%7B%5Bk%5D%7D "\beta_{[k]}"))
...
(frequentist) coverage probabilities (5).
...
| 1970s difference |    0.98 | 3.30 |    6.21 |   0.000 |
...
class="csl-right-inline">S. N. Wood, [Stable and efficient multiple
smoothing parameter estimation for generalized additive
models](https://doi.org/10.1198/016214504000000980). *Journal of the
American Statistical Association* **99**, 673–686 (2004).</span>
...
class="csl-right-inline">S. N. Wood, [Low-rank scale-invariant tensor
product smooths for generalized additive mixed
models](https://doi.org/10.1111/j.1541-0420.2006.00574.x). *Biometrics*
**62**, 1025–1036 (2006).</span>
...
class="csl-right-inline">S. N. Wood, [Fast stable restricted maximum
likelihood and marginal likelihood estimation of semiparametric
generalized linear
models](https://doi.org/10.1111/j.1467-9868.2010.00749.x). *Journal of
...
class="csl-right-inline">S. N. Wood, N. Pya, B. Säfken, [Smoothing
parameter and model selection for general smooth
models](https://doi.org/10.1080/01621459.2016.1180986). *Journal of the
American Statistical Association* **111**, 1548–1563 (2016).</span>
...
class="csl-right-inline">G. Marra, S. N. Wood, [Coverage properties of
confidence intervals for generalized additive model
components](https://doi.org/10.1111/j.1467-9469.2011.00760.x).
*Scandinavian Journal of Statistics* **39**, 53–74 (2012).</span>
...
class="csl-right-inline">S. N. Wood, [On p-values for smooth components
of an extended generalized additive
model](https://doi.org/10.1093/biomet/ass048). *Biometrika* **100**,
221–228 (2013).</span>
